...
**重要性采样权重（Probability Ratio）:**
$$r_t(\theta) = \frac{\pi_\theta(y_t | x, y_{<t})}{\pi_{\text{old}}(y_t | x, y_{<t})}$$

它量化了在给定上下文的情况下，新策略生成当前Token的概率相对于旧策略的变化倍数。

$\hat{A}_t$：这是**优势函数（Advantage Function）**的估计值，通常使用广义优势估计（Generalized Advantage Estimation, GAE）来计算。$\hat{A}_t$ 衡量了动作 $y_t$ 相对于平均水平（由价值函数 $V(s)$ 预测）的优劣程度。若 $\hat{A}_t > 0$，说明当前动作优于预期，模型应增大其概率；若 $\hat{A}_t < 0$，说明当前动作劣于预期，模型应减小其概率。

$\text{clip}(1-\epsilon, 1+\epsilon)$：这是PPO的灵魂所在。$\epsilon$ 通常取值为0.1或0.2。这个裁剪操作定义了一个**“信任域”（Trust Region）**，强制要求新旧策略的概率比率维持在 $[1-\epsilon, 1+\epsilon]$ 的区间内。这防止了单次更新步长过大导致策略崩溃（Policy Collapse），即防止模型因为一次偶然的高奖励而过度拟合到某个特定的轨迹上。
...
$$
r(s_t, a_t) = R(x, y) \cdot \mathbb{I}(t=T) - \beta \log \frac{\pi_\theta(a_t|s_t)}{\pi_{ref}(a_t|s_t)}
$$

将KL散度作为逐Token的即时惩罚，而非仅在序列末尾结算。
...
2.贪婪轨迹（确定性，代表当前最佳水平）。
$$\hat{y}_{greedy} = \text{argmax} \pi_\theta(\cdot|x)$$
...
**AutoThink** 是一种创新的强化学习框架，旨在解决大型推理模型（LRMs）中常见的**过度思考（Overthinking）**问题，即模型在回答简单问题时也产生大量冗余的推理步骤。

该框架的核心思想是利用简单的**省略号提示词（Ellipsis Prompt）**触发模型的双模态（Thinking/No-Thinking）潜能，并通过**三阶段的奖励重塑（Reward Shaping）**机制，训练模型根据问题的复杂度自适应地分配**推理算力（Test-time Compute）**。
...
AutoThink 发现了一个令人惊讶的现象：在 R1 类模型（如 DeepSeek-R1）中，只需将提示词修改为极简形式 `<think>\n...\n`，即可作为“开放信号”，随机触发模型进入“思考模式”（生成详细推理步骤）或“不思考模式”（直接给出答案）。这种随机性为后续的策略学习提供了探索空间。

#### 基础优化算法：GRPO
AutoThink 采用群组相对策略优化（Group Relative Policy Optimization, GRPO）作为底层 RL 算法。其目标函数如下：
...
在初始阶段，如果直接训练，模型倾向于坍塌到单一模式（例如：为了短期高回报而总是选择不思考，或总是思考）。本阶段通过**Batch 级奖励平衡**来强行维持两种模式的共存。

**1. 朴素奖励定义 (Naive Reward)**
首先定义基础奖励 $r_{i}^{naive}$，鼓励“不思考且正确”（最高效）的行为 ：
...
引入变量 $z \in [0,1]$ 表示当前 Batch 中“思考模式”的占比。设定目标比例 $\gamma$（如 0.5）和惩罚斜率 $\lambda$。
...
最终奖励 $r_{i}^{adj}$ 会根据当前 Batch 中哪种模式“过多”而降低该模式的奖励，迫使模型探索另一种模式：
...
在此阶段，通常会增加训练的上下文长度预算（Context Budget），允许模型在需要时进行更深度的推理。
...
这是 AutoThink 的核心差异化阶段。引入**长度感知（Length-Aware）**奖励，惩罚冗余的思考，鼓励在错误时进行更深入的反思。
...
**2. 长度感知奖励函数**
引入超参数 $\alpha$ 和 $\beta$ 来控制整形敏感度。调整后的奖励如下：
...
* **对于错误回答 ($correct=0$)**：使用 $(1 - e^{-\beta y_{i}})$ 项。
    ...
    * **效果**：如果模型答错了，但进行了长距离思考，给予一定宽容；反之，如果“秒答且错”（Lazy failure），则得不到补偿。这鼓励模型在遇到困难时不要轻易放弃思考 。

### 7.3 AutoThink 的演进逻辑与成效

| 阶段 | 核心任务 | 奖励机制特征 | 预期行为结果 |
| :--- | :--- | :--- | :--- |
| **Stage 1** | **稳定化** | Batch 级动态平衡 ($\gamma, \lambda$) | 模型学会既能“思考”，也能“不思考”，不偏科。 |
| **Stage 2** | **强化** | 纯粹的正确性奖励 ($r^{naive}$) | 模型在两种模式下的解题准确率都大幅提升。 |
| **Stage 3** | **剪枝** | 长度感知奖励 ($\alpha, \beta$) | **自适应涌现**：简单题自动切换为短回复（不思考），难题保留长推理。 |
