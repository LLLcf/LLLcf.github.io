...
# 大模型强化学习方法综述：从对齐到推理的演进之路

## 1. 引言：从人类偏好对齐到系统性推理能力的觉醒
...
在这个新的“后训练”（Post-training）时代，RL不再仅仅是风格的过滤器，而是成为了能力的放大器。特别是在数学推导、代码生成和逻辑推理等具有确切“真值”（Ground Truth）的领域，基于**可验证奖励的强化学习（RLVR）**使得模型能够通过自我博弈和试错，涌现出类似人类“系统2”思维的长链推理能力（Chain-of-Thought, CoT）。这种能力的提升并非源于灌输更多的知识，而是源于RL算法激励模型在推理空间中进行更深度的搜索、反思与自我修正。
...
PPO的核心在于其目标函数的设计，该函数引入了一个裁剪（Clipping）机制，用于限制新策略 $\pi_\theta$ 与旧策略 $\pi_{\text{old}}$ 之间的差异。其优化目标定义为：
...
尽管PPO在OpenAI的InstructGPT等早期工作中取得了巨大成功，但随着模型参数量的爆炸式增长（从7B到70B乃至671B），PPO的架构缺陷逐渐暴露无遗，成为了制约大模型推理能力扩展的瓶颈：
...
**DPO的局限性：** 尽管DPO极大地简化了RLHF流程并被广泛应用（如Llama 3, Qwen 2.5的对齐），但在推理能力的提升上，它存在先天不足。DPO本质上是一种离线算法，它依赖于预先收集的偏好数据对 $(y_w, y_l)$。这意味着模型只能学习数据集中已有的推理模式，而无法像在线RL（Online RL）那样通过探索（Exploration）发现数据集中不存在的新颖解题路径。对于需要“顿悟”（Aha Moment）和深度搜索的推理任务，DPO往往只能做“表面文章”，难以实现真正的能力突破。
...
1.  采样轨迹 $y \sim \pi_\theta(\cdot|x)$（带有随机性，用于探索）。
2.  贪婪轨迹 $\hat{y}_{greedy} = \text{argmax} \pi_\theta(\cdot|x)$（确定性，代表当前最佳水平）。
...
如果说PPO是RLHF时代的引擎，那么DeepSeek-AI提出的**群体相对策略优化（Group Relative Policy Optimization, GRPO）**无疑是开启大模型推理时代（LRM Era）的关键钥匙。DeepSeek-R1及其前身DeepSeek-Math正是依靠GRPO实现了令人惊叹的推理能力飞跃。
...
**算法流程：**
1.  **群体采样（Group Sampling）**：对于每个Prompt $x$（通常来源于数学或代码题目），从旧策略 $\pi_{\theta_{old}}$ 中采样一组 $G$ 个输出 $\{y_1, y_2,..., y_G\}$。
2.  **奖励计算**：对每个输出计算奖励 $r_1,..., r_G$。这些奖励通常是二值的（正确/错误）或基于规则的评分。
3.  **优势估计（Group-based Advantage）**：利用组内的统计量来标准化奖励，作为优势函数的估计：
...
4.  **策略更新**：最大化以下目标函数：
    ...
    其中，$w_{i,t}(\theta) = \frac{\pi_\theta(y_{i,t} | x, y_{i,<t})}{\pi_{old}(y_{i,t} | x, y_{i,<t})}$ 是重要性采样权重。

### 4.2 GRPO相对于PPO的关键变革

| 特性 | PPO (传统RLHF) | GRPO (DeepSeek-R1) | 优势/洞察 |
| :--- | :--- | :--- | :--- |
...
随着模型架构向混合专家（Mixture-of-Experts, MoE）演进（如Qwen2.5-VL, DeepSeek-V3），以及对长链推理稳定性要求的提高，GRPO在Token级别的优化方式开始显露疲态。

### 5.1 GSPO：群组序列策略优化

由阿里巴巴Qwen团队提出的GSPO（Group Sequence Policy Optimization）是对GRPO的一次重要修正，旨在解决MoE模型训练中的稳定性问题。
...
### 6.2 过程奖励（Process Rewards）：PRM与PRIME

仅仅依靠最终结果（Outcome Reward）有时反馈过于稀疏，特别是对于几十步的复杂推理。
* **PRM (Process Reward Model)**：针对推理的每一步打分（例如Math-Shepherd项目）。但这需要昂贵的人工过程标注。
* **PRIME (Process Reinforcement through Implicit Rewards)**：这是一种极具创新性的方法。PRIME不需要显式的过程标注。它基于一个洞察：一个状态的价值（Value）本质上就是它能通向正确结果的概率。PRIME利用一个仅在结果标签上训练的模型，隐式地推导出中间Token的价值，从而实现了密集奖励（Dense Reward）的效果，填补了稀疏结果奖励与昂贵过程监督之间的空白。

---

## 7. 自适应推理：AutoThink框架

在追求更强推理能力的同时，另一个趋势是追求效率。我们不希望模型在回答“1+1等于几”时也进行长篇大论的推理。

AutoThink提出了一种基于多阶段RL的自适应推理框架：
1.  **Stage 1（稳定化）**：使用特殊的“省略号Prompt”（`<think>...`）随机触发思考或不思考模式，通过Batch级的奖励平衡，防止模型坍塌到单一模式。
2.  **Stage 2（强化）**：在两种模式下都强化正确性。
3.  **Stage 3（剪枝）**：引入长度感知奖励（Length-Aware Reward）。对于回答正确的情况，如果长度过长，给予惩罚；对于回答错误的情况，如果长度过短（思考不足），给予惩罚。

这使得模型学会了根据问题的难易程度，动态分配测试时算力（Test-time Compute）。
...
未来的方向将是过程监督的自动化（如PRIME）与自适应计算（如AutoThink）的结合。我们正在见证AI从单纯的模式匹配者，进化为真正的思考者，而强化学习正是驱动这一进化的核心引擎。
