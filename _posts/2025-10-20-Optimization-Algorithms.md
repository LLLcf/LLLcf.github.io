...
在实际训练中，我们处理的是小批量（Mini-batch）数据 $\mathbf{X} \in \mathbb{R}^{B \times d_{in}}$。此时，梯度计算变为矩阵乘法，极大地利用了 GPU 的并行能力。
...
$\mathbf{G}_t$ 是历史梯度的平方和。在文本数据中，低频词对应的参数 $\mathbf{g}_t$ 大多为 0，$\mathbf{G}_t$ 增长缓慢，因此保持了较大的有效学习率。

* **局限**：$\mathbf{G}_t$ 单调递增，导致分母无限变大，学习率在训练后期会过早衰减至 0，导致欠拟合。
...
4.  偏差修正（Bias Correction）：由于 $\mathbf{m}, \mathbf{v}$ 初始化为 0，早期估计偏向 0。
    $$\hat{\mathbf{m}}_t = \frac{\mathbf{m}_t}{1-\beta_1^t}$$
    $$\hat{\mathbf{v}}_t = \frac{\mathbf{v}_t}{1-\beta_2^t}$$
5.  参数更新：
    $$\mathbf{w}_{t+1} = \mathbf{w}_t - \eta \frac{\hat{\mathbf{m}}_t}{\sqrt{\hat{\mathbf{v}}_t} + \epsilon}$$

Adam 的物理意义在于：它根据梯度的**信噪比（Signal-to-Noise Ratio）**调整步长。$\hat{\mathbf{m}}_t$ 代表信号，$\sqrt{\hat{\mathbf{v}}_t}$ 代表噪声（波动）。信噪比高的方向步长更大，反之则小。
...
在训练巨型模型时，Adam 优化器需要存储 $\mathbf{m}$ 和 $\mathbf{v}$ 两个状态矩阵，这意味着优化器显存占用是模型参数的 2 倍（如果是 FP32 状态甚至是 3 倍）。

Adafactor 通过矩阵分解技术解决了这一问题。它不再存储完整的 $n \times m$ 二阶矩矩阵 $\mathbf{V}$，而是将其近似为行向量 $\mathbf{R} \in \mathbb{R}^n$ 和列向量 $\mathbf{C} \in \mathbb{R}^m$ 的外积：
...
* **显存优势**：不需要存储二阶矩 $\mathbf{v}$，比 AdamW 节省 33%-50% 显存。
...
Adam 用梯度平方近似 Hessian 对角线，这在理论上是粗糙的。Sophia 使用一种轻量级的对角 Hessian 估计器 $\hat{\mathbf{h}}$，并结合**元素级裁剪（Clipping）**来防止更新爆炸：
...
AdamW、Lion 等算法在处理权重矩阵 $\mathbf{W} \in \mathbb{R}^{d_{out} \times d_{in}}$ 时，本质上将其视为长度为 $d_{out} \times d_{in}$ 的扁平向量。这种处理方式忽略了矩阵的谱结构（Singular Value Structure）。
...
1.  **动量积累（标准 SGD）**：
    ...
    （这里 $\mathbf{G}_t$ 是当前 Batch 的梯度）

2.  **正交化（Orthogonalization）**：
    我们需要找到一个正交矩阵 $\mathbf{O}_t$，使得它在某种意义上最接近 $\mathbf{M}_t$。数学上，这等价于取 $\mathbf{M}_t$ 的 SVD 分解 $\mathbf{U}\boldsymbol{\Sigma}\mathbf{V}^\top$，然后令 $\mathbf{O}_t = \mathbf{U}\mathbf{V}^\top$。
...
| 特性 | Shampoo | Muon |
| :--- | :--- | :--- |
| **核心机制** | 预调节器（Preconditioner）基于 Kronecker 积 $\mathbf{L}^{-1/4} \mathbf{G} \mathbf{R}^{-1/4}$ | 更新方向正交化 $\text{Newton-Schulz}(\mathbf{M})$ |
...
2.  计算 $\mathbf{O}_t$ 的列方向 RMS 统计量 $\mathbf{v}_t$。
